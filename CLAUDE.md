...
**Last Updated**: 2026-08-08 (v3.4.7)

> 本文档已同步 v3.4.7 的自动 Provider 网页预览、只读 wrapper 调用与 Antigravity 结构化中间输出；较早章节仍保留历史架构背景，完整历史见 [CHANGELOG.md](./CHANGELOG.md)。
...
- Context7、Playwright、官方 DeepWiki 和 Exa 进入只读推荐目录，默认不选，
  统一由 `ccg config mcp` 承接逐项配置。
- DeepWiki 与 Exa 的官方 Streamable HTTP 端点按 Claude、Codex、Gemini
  原生字段镜像；远程 URL 严格 allowlist，Exa Key 继续使用 owner-only
  secret launcher。
...
| codeagent-wrapper | [codeagent-wrapper/CLAUDE.md](./codeagent-wrapper/CLAUDE.md) | Go 二进制包装器，多模型调用桥接，v5.12.7 |
...
graph TD
    ...
    Init --> Binary["~/.claude/bin/<br/>codeagent-wrapper v5.12.7"]
