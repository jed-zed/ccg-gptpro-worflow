...
function validRunnerResult() {
  return {
    exitCode: 0,
    status: 'valid',
    evidencePath: '.codex/ccg/intelligence/evidence/evidence.json',
    evidenceSha256: 'a'.repeat(64),
    manifestPath: '.codex/ccg/intelligence/evidence/manifest.json',
...
describe('Grok automatic intelligence routing', () => {
  ...
  it('routes a dependency or API contract intake before the workflow and persists call order', async () => {
    ...
    expect(invocation.argv).toEqual([
      'intel', '--task', 'Upgrade the Acme SDK and verify its current API contract.',
      '--mode', 'contract', '--dependency', 'pnpm-lock.yaml',
    ])
  ...
  it('routes a current incident with required Web evidence and policy-derived X evidence', async () => {
    ...
    const result = await (routeRuntime as any).runWorkflowRoute({
      ...
      stateFile: join(root, 'incident-state.json'),
    }, {
      invoke: async (request: any) => {
        invocation = request
        return validRunnerResult()
    ...
    expect(invocation.argv).toEqual([
      'intel', '--task', 'Diagnose the current Acme Cloud outage and recent regional errors.', '--mode', 'incident',
    ])
  })
  ...
  it.each([
    ...
    let calls = 0
    const stateFile = join(root, `${String(_label).replace(/\s/g, '-')}.json`)
    const result = await (routeRuntime as any).runWorkflowRoute({
      repoRoot: root,
      config,
      workflow: 'go',
  ...
  it('records a skip reason for task classes outside the initial automatic gates', async () => {
    ...
    const result = await (routeRuntime as any).runWorkflowRoute({
      ...
      stateFile: join(root, 'local-skip.json'),
    }, {
      invoke: async () => { calls++; return validRunnerResult() },
    })
  ...
  it('supports an explicit Codex semantic decision without requiring search words in the task', async () => {
    ...
    const result = await (routeRuntime as any).runWorkflowRoute({
      ...
      stateFile: join(root, 'semantic-contract.json'),
    }, {
      invoke: async (request: any) => { invocation = request; return validRunnerResult() },
    })
  ...
  it('reuses unchanged final verification and invalidates it on diff or dependency digest changes', async () => {
    const stateFile = join(root, 'verify-state.json')
    let calls = 0
  ...
  it('propagates required evidence exit 2 and records the blocking result', async () => {
    const stateFile = join(root, 'blocked-state.json')
    const result = await (routeRuntime as any).runWorkflowRoute({
      repoRoot: root,
      config: enabledConfig(),
      workflow: 'go',
  ...
  it('publishes a validated successful bundle into canonical task evidence and pointer state', async () => {
    ...
    const artifact = `${JSON.stringify({
      schemaVersion: 2,
      decision: {
        requirement: 'required', status: 'valid', mode: 'contract',
        reason: 'Current contract verified.', created_at: '2026-07-21T00:00:00.000Z',
      },
      evidence: { claims: [] },
    }, null, 2)}\n`
    ...
    const manifest = `${JSON.stringify({
      ...
      localOnly: true,
      exported: false,
      files: { 'evidence.json': { sha256: artifactSha256, bytes: Buffer.byteLength(artifact) } },
    }, null, 2)}\n`
  ...
  it('rejects plan and dependency paths that escape through a link or junction', async () => {
    ...
    await expect((routeRuntime as any).runWorkflowRoute({
      ...
      stateFile: join(root, 'linked-state.json'),
    }, {
      invoke: async () => { calls++; return validRunnerResult() },
